 ...
 * re-doing the in-flight container idempotently). Older runs persisted a
 * generation-based `{ phase: "documents" }` shape — those are ignored and the
 * pass starts fresh.
 */
...
class PermissionSyncService {
  ...
  async executePass(
    ...
    // rather than restarting the reconcile. Pre-container checkpoint shapes
    // (the retired generation-based reconcile) are ignored.
    const adoptedCheckpoint = (run.checkpoint ??
    ...
    const priorCheckpoint =
      adoptedCheckpoint?.phase === "snapshot" ? adoptedCheckpoint : null;
    ...
    try {
      ...
      if (connectorImpl.syncGroups) {
        ...
        try {
          ...
          // Completion-gated diff delete of revoked memberships.
          const revoked = [...currentByKey.keys()]
            .filter((key) => !seen.has(key))
            .map((key) => {
              const [groupId, externalAccountId] = key.split("\u0000");
              return { groupId, externalAccountId };
            });
      ...
      if (generator) {
        for await (const item of generator) {
          ...
          if (item.kind === "container") {
            ...
            const changed = await this.upsertContainer({
              connector,
            ...
            if (changed)
              stats.containersChanged = (stats.containersChanged ?? 0) + 1;
          } else {
            stats.docsScanned += 1;
  ...
  /**
   * Upsert one container's audience row. The audience ACL is built through
   * the same authority as document ACLs (cap + `org:*` over-approximation
   * included). Returns whether the stored audience actually changed — the
   * pass's headline number, since ONE changed container row is the entire
   * write cost of an upstream audience change.
   */
  private async upsertContainer(params: {
    connector: KnowledgeBaseConnector;
    containerKey: string;
    permissions: unknown;
    fingerprint: string | null;
  }): Promise<boolean> {
    const { connector, containerKey, permissions, fingerprint } = params;
    ...
    const existing = await KbContainerAclModel.findAudiencesByKeys({
      connectorId: connector.id,
      containerKeys: [containerKey],
    });
    const previous = existing.get(containerKey);
    const changed = previous === undefined || !aclEquals(previous, acl);

    // Upserted even when unchanged: the re-upsert clears `stale`, which is
    // what protects the container from the vanished-container sweep.
  ...
  private async flushAssignments(params: {
    ...
    for (const doc of current) {
      ...
      // container assignment) last. Both epoch-fenced. Not transactional, but
      // self-healing: a crash between the two leaves the doc row stale, so
      // the next pass re-detects the diff (this compare reads the doc row)
      // and redoes both writes — the chunk update is idempotent.
      stats.chunksRewritten += await KbChunkModel.updateAclByDocument({
        documentId: doc.id,
        acl: nextAcl,
        connectorId: connector.id,
        aclConfigEpoch,
      });
      const wrote = await KbDocumentModel.updateDocumentAssignment({
        documentId: doc.id,
        connectorId: connector.id,
        acl: nextAcl,
        containerKey: assignment.containerKey,
