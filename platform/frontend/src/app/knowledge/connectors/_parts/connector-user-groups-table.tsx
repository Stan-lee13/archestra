...
/**
 * Members rendered like the documents table's Access column: the first two
 * as outline badges, the rest collapsed behind a "+N more" badge whose
 * tooltip lists all of them (scrollable). App/bot accounts are excluded —
 * they never sign in, so they are noise here (the Assigned counts skip
 * them too).
 */
function GroupMemberBadges({ group }: { group: ConnectorUserGroup }) {
  ...
  const visible = humans.slice(0, MAX_VISIBLE_MEMBERS);
  const hidden = humans.slice(MAX_VISIBLE_MEMBERS);
  return (
